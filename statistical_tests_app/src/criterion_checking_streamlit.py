...
class CriterionCheck:
    ...
    def shapiro_uilk(self):
        ...
        """
        stat, p = stats_.shapiro(self.sample)
        if p < self.alpha:
            st.write(f'Статистика по выборке {self.title} - {stat:.2f}. P-value - {p:.2f}. Отклонить гипотезу о нормальности распределения.')
        else:
           st.write(f'Статистика по выборке {self.title} - {stat:.2f}. P-value - {p:.2f}. Принять гипотезу о нормальности распределения.')
        return stat, p
    ...
    def test_leven(self):
        ...
        """
        stat, p = stats_.levene(*[np.asarray(s).flatten() for s in self.samples]) # DataFrame сворачивается в одномерный массив
        if p < self.alpha:
            st.write(f'Статистика Левена по выборкам - {stat:.2f}. P-value - {p:.2f}. Отвергаем гипотезу о гомогенности дисперсий выборок.')
        else:
            st.write(f'Статистика Левена по выборкам - {stat:.2f}. P-value - {p:.2f}. Принимаем гипотезу о гомогенности дисперсий выборок.')
        return stat, p
    ...
    def post_analysis_anova_tukey(self):
        ...
        st.dataframe(tukey_post_result.summary())
        # здесь возможно расширить функционал выводом диаграммы при применении пост-анализа
