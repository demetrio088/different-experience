...
class NonparametricTest:
    ...
    def mann_uitni(self):
        ...
        if p < self.alpha:
            st.write(f'**U-критерий Манна-Уитни по группам выборок - {u_stat:.2f}. P-value - {p:.2f}. Отвергаем нулевую гипотезу о схожести медиан/распределений двух групп.**')
        else:
            st.write(f'**U-критерий Манна-Уитни по группам выборок - {u_stat:.2f}. P-value - {p:.2f}. Не отвергаем нулевую гипотезу о схожести медиан/распределений двух групп.**')
        return u_stat, p
    ...
    def wilcoxon(self):
        ...
        """
        w_stat, p = stats_.wilcoxon(*[np.asarray(s).flatten() for s in self.samples], alternative=self.test_type)
        if p < self.alpha:
            st.write(f'**Критерий Вилкоксона по группам выборок - {w_stat:.2f}. P-value - {p:.2f}. Отвергаем нулевую гипотезу о схожести рангов разностей пары.**')
        else:
            st.write(f'**Критерий Вилкоксона по группам выборок - {w_stat:.2f}. P-value - {p:.2f}. Не отвергаем нулевую гипотезу о схожести рангов разностей пары.**')
        return w_stat, p
    ...
    def kolmogorov_smirnov(self):
        ...
        """
        d_stat, p = stats_.kstest(*[np.asarray(s).flatten() for s in self.samples], alternative=self.test_type)
        if p < self.alpha:
            st.write(f'**Критерий Колмогорова-Смирнова по группам выборок - {d_stat:.2f}. P-value - {p:.2f}. Отвергаем нулевую гипотезу. Выборки, вероятно, имеют разные распределения.**')
        else:
            st.write(f'**Критерий Колмогорова-Смирнова по группам выборок - {d_stat:.2f}. P-value - {p:.2f}. Не отвергаем нулевую гипотезу. Выборки, вероятно, имеют одинаковое распределение.**')
        return d_stat, p
