...
class ParametricTest:
    ...
    def student_test_one_sample(self):
        ...
        """
        stat, p = stats_.ttest_1samp(self.sample_1, self.sample_mean, alternative=self.test_type)
        if p < self.alpha:
            st.write(f'**Статистика Стьюдента по выборке и среднему - {stat:.2f}. P-value - {p:.2f}. Отвергаем нулевую гипотезу: среднее значение выборки отличается от предполагаемого среднего.**')
        else:
            st.write(f'**Статистика Стьюдента по выборке и среднему - {stat:.2f}. P-value - {p:.2f}. Не отвергаем нулевую гипотезу: нет оснований утверждать, что среднее выборки отличается от предполагаемого среднего.**')
        return stat, p
    ...
    def student_test_two_samples(self):
        ...
        Ограничения по размеру выборки: n > 5.
        """
        stat, p = stats_.ttest_ind(*[np.asarray(s).flatten() for s in self.samples], alternative=self.test_type, equal_var=self.equal_var)
        if p < self.alpha:
            st.write(f'**Статистика Стьюдента по выборкам - {stat:.2f}. \n P-value - {p:.2f}. Отвергаем нулевую гипотезу.** \n')
        else:
            st.write(f'**Статистика Стьюдента по выборкам - {stat:.2f}. \n P-value - {p:.2f}. Не отвергаем нулевую гипотезу** \n')
        return stat, p
    ...
    def anova_one(self):
        ...
        """
        f_stat, p = stats_.f_oneway(*[np.asarray(s).flatten() for s in self.samples])
        
        if p < self.alpha:
            st.write(f'**F-критерий ANOVA по группам выборок - {f_stat:.2f}. P-value - {p:.2f}. Отвергаем нулевую гипотезу о равенстве средних.**')
        else:
            st.write(f'**F-критерий ANOVA по группам выборок - {f_stat:.2f}. P-value - {p:.2f}. Не отвергаем нулевую гипотезу о равенстве средних.**')
        return f_stat, p    
